...
import type { EvaluationType, Prisma } from '@prisma/client';
...
// ── Evaluaciones generadas por IA + asignaciones ────────────────

export async function createGeneratedEvaluation(params: {
  companyId: string;
  jobId: string;
  title: string;
  type: EvaluationType;
  ...
  const { companyId, jobId, title, type, questions } = params;
  return db.$transaction(async (tx) => {
    const evaluation = await tx.evaluation.create({
      data: {
        ...
        description: 'Prueba generada automáticamente a partir del análisis del cargo.',
      },
    });
    // La deja asignada a la vacante de la que se generó.
    await tx.jobEvaluation.upsert({
      where: { jobId_evaluationId: { jobId, evaluationId: evaluation.id } },
      update: {},
      create: { companyId, jobId, evaluationId: evaluation.id },
    });
    return evaluation;
  });
}
...
export async function createAssignment(params: {
  ...
  return db.evaluationAssignment.create({ data: { ...params, status: 'PENDING' } });
}

export async function getAssignmentByToken(token: string) {
  return db.evaluationAssignment.findUnique({
    where: { token },
    include: { evaluation: true, candidate: { select: { fullName: true } } },
  });
}

export async function listAssignmentsForCandidate(companyId: string, candidateId: string) {
  return db.evaluationAssignment.findMany({
    where: { companyId, candidateId },
    include: { evaluation: { select: { title: true, type: true } } },
    orderBy: { createdAt: 'desc' },
  });
}

/** Evaluaciones generadas por IA (con preguntas) de la empresa. */
export async function listGeneratedEvaluations(companyId: string) {
  return db.evaluation.findMany({
    where: { companyId, deletedAt: null, autoGenerated: true },
    select: { id: true, title: true, type: true, jobId: true },
    orderBy: { createdAt: 'desc' },
  });
}

export async function markAssignmentCompleted(assignmentId: string) {
  return db.evaluationAssignment.update({
